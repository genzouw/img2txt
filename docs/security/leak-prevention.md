...
1. **コミット前検知（ローカル開発・AIエージェント環境）**:
   ...
   - カスタムルール: リポジトリルートの `.gitleaks.toml` にて、GCP Project ID（例: `<REDACTED>` 等）や Neon Postgres エンドポイント、Cloud Run エンドポイント（`*.run.app`）、Cloudflare Pages エンドポイント（`*.pages.dev`）、Workload Identity Federation プロバイダ文字列等のインフラ構成の過剰露出を検知・拒否する独自ルールを追加しています。また、追加のクラウド識別子（GCP Project Number, Service Account メールアドレス）、内部 IP アドレス、PII（本物のメールアドレス）、特定の API キー（Resend）についてもハードコードを禁止し、AI エージェントの作業跡（`.cursor/`, `.claude/` 等）や `.env` ファイル、`credentials.json` などの秘匿性が高いファイルそのものがコミットされることを防ぐため、また IaC の state ファイル（`cdktf.out/`, `*.tfstate` 等）や変数ファイル（`*.tfvars` 等）を除外するため、ファイルパスベースの検知ルールを導入しています。今回、新たに SSH 秘密鍵（`id_rsa`, `id_ed25519` 等）やローカル認証設定ファイル（`.npmrc`, `.netrc`, `.aws/credentials` 等）の混入を厳格に防ぐカスタムルールを追記しました。さらに、Postgres や Redis などの完全な接続文字列のハードコードを禁止し、AI エージェントが残しがちなデバッグ用の一時値（`YOUR_API_KEY` や `dummy_secret` など）も検知・拒否対象としています。直近のアップデートにて、**内部インフラの過剰露出（`*.internal`, `*.corp` などの内部ドメイン）**、**HTTP/HTTPS URLへのBasic認証情報の埋め込み（`https://user:pass@domain`）**、および**AI エージェント用プレースホルダーの拡張（`CHANGE_ME` や `XXX_SECRET_XXX` 等）**を厳格に検知・拒否するルールを追加しました。ダミー値やマスクが必要な場合は、プレースホルダーとして `<REDACTED>` を使用してください。開発時はこれらの識別子を直接コードに書かず、環境変数やシークレット管理サービスを経由して参照するようにしてください。
